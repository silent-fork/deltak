...
function Metric({
  ...
  return (
    <div className="rounded border border-zinc-800 bg-zinc-950/50 px-2 py-1.5" title={title}>
      <div className="dk-label">{label}</div>
      <div className={cn("mt-0.5 font-mono text-xs font-semibold", tone ?? "text-zinc-100")}>
        {value}
      </div>
    </div>
  );
...
export function SignalPanel({
  ...
  async function execute() {
    ...
    setResult(null);
    try {
      const res = await api.executeSignal({
          ...
          <div className="rounded border border-zinc-800 bg-zinc-950/60 p-2">
            ...
            {signal.sizing.capped_by ? (
              <div className="mt-1 text-[9px] uppercase tracking-wider text-amber-400/80">
                Capped · {BLOCK_REASONS[signal.sizing.capped_by] ?? signal.sizing.capped_by}
              </div>
            ) : null}
          </div>
        ) : null}
