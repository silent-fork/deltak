...
export const FAQS: FaqItem[] = [
  ...
  },
  {
    ...
    answer: [
      ...
      "What did hold up on repeated, independent re-runs was execution quality, not signal re-tuning: the exact same unchanged signal, run once with only the limit-entry and thesis-exit mechanics above turned on, moved from roughly 40% win rate and a real loss to a strongly positive expectancy — with the held-out test fold (the one no parameter selection ever touched) coming back stronger than the training fold, not weaker, which is the opposite of an overfit result.",
      "The current shipped configuration backtests to an annualised Sharpe ratio of roughly 6.1 and a maximum drawdown of about −11% on real position sizing, a 71.8% win rate across 692 trades, and turns ₹1,00,000 of paper capital into roughly ₹1.07 crore over the study window. Full attribution by index, protocol and exit reason — plus the risks this doesn't paper over — is in the backtest report.",
    ],
