...
const MARKET_SNAPSHOTS = "market_snapshots";

/**
 * The board's last-available data for one underlying, while the market is
 * closed. Global — keyed on the underlying alone, not a client code — since
 * this describes the market, not an account. Overwritten on the market
 * closing and on each focus switch while it stays closed; see the migration
 * comment for the two write sites.
 */
export async function writeMarketSnapshot(
  ...
  if (!supabaseConfigured || !underlying) return;
  const res = await fetch(`${base()}/${MARKET_SNAPSHOTS}?on_conflict=underlying`, {
    method: "POST",
    headers: headers("resolution=merge-duplicates,return=minimal"),
    body: JSON.stringify([
      {
  ...
  if (!res.ok) {
    throw new Error(
      `Supabase market-snapshot write failed (${res.status}): ${(await res.text()).slice(0, 200)}`,
    );
  }
}
...
export async function readMarketSnapshot(
  underlying: string,
): Promise<{ session_date: string; snapshot: unknown; updated_at: string } | null> {
  if (!supabaseConfigured || !underlying) return null;
  const search = new URLSearchParams({
    select: "session_date,snapshot,updated_at",
    underlying: `eq.${underlying}`,
    limit: "1",
  });
  const res = await fetch(`${base()}/${MARKET_SNAPSHOTS}?${search.toString()}`, {
    headers: headers(),
    cache: "no-store",
  });
  if (!res.ok) return null;
  const body = await res.json();
  return Array.isArray(body) && body.length
    ? (body[0] as { session_date: string; snapshot: unknown; updated_at: string })
    : null;
}
